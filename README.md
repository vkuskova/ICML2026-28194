...
> Paper: *Function-Valued Causal Influence in Nonlinear Time Series* (ICML 2026, Paper #28194)  
> arXiv: [link to be added after acceptance]  
> DOI: [link to be added after proceedings publication]
...
This repository contains the code used to produce all figures and tables in the paper.
The core contribution is a framework that estimates **function-valued causal influence**
from trained additive autoregressive models using Individual Conditional Expectation (ICE),
revealing regime-dependent causal mechanisms that scalar causal scores cannot capture.
...
|---|---|
| `README.md` | This file |
| `ICML_28194_code.ipynb` | Main Colab notebook — all figures and tables |
| `requirements.txt` | Python dependencies |
| `NAVAR.py` | Neural Additive Vector Autoregression model architecture |
| `train_NAVAR.py` | NAVAR training loop |
| `dataloader.py` | Data loading utilities for NAVAR |
| `evaluate.py` | Evaluation utilities |
| `edge_ablation.py` | Forecast-necessity edge ablation (Diebold-Mariano test) |

**Note:** The democracy dataset (`my_data.csv`) is derived from the
[V-Dem v15 dataset](https://www.v-dem.net) (Coppedge et al., 2025,
DOI: [10.23696/vdemds25](https://doi.org/10.23696/vdemds25)) and is not
included in this repository. Instructions for obtaining and preprocessing
it are in the notebook and in Appendix E of the paper.
...
└── ICML 28194/
    ├── code/          ← upload all .py files from this repo here
    ├── data/
    │   └── my_data.csv   ← V-Dem panel data (see below)
    └── results/       ← outputs will be saved here automatically
```
...
1. Upload `ICML_28194_code.ipynb` to Colab (File → Upload notebook)
2. Upload all `.py` files from this repo to `MyDrive/ICML 28194/code/`
3. Prepare `my_data.csv` (see Data section below) and place in `MyDrive/ICML 28194/data/`
4. Run cells in order, starting with **Cell 0** (Drive mount)
5. After **Cell 1** (dependency install), restart the Colab runtime before continuing
...
V-Dem data is freely available at [v-dem.net](https://www.v-dem.net).
The specific variables used are listed in Appendix E, Table E.1 of the paper.
After downloading, preprocess according to Appendix E (filter to 139 countries
with complete 35-year coverage, 1990–2024; standardize all variables to zero
...
The NAVAR architecture (`NAVAR.py`, `train_NAVAR.py`, `dataloader.py`) is adapted
from [Bussmann et al. (2021)](https://arxiv.org/abs/2105.01890) and is used under
its original license terms.

V-Dem data is released under
[CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) by the V-Dem Institute.
